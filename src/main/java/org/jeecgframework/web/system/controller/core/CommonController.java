...
public class CommonController extends BaseController {
	...
	public ModelAndView openViewFile(HttpServletRequest request) throws FileNotFoundException {
		String fileid = request.getParameter("fileid");
		String subclassname = oConvertUtils.getString(
				request.getParameter("subclassname"),
				"org.jeecgframework.web.system.pojo.base.TSAttachment");
		String contentfield = oConvertUtils.getString(request
				.getParameter("contentfield"));
		...
		String extend = oConvertUtils.getString(reflectHelper
				.getMethodValue("extend"));

		String strRealPath = oConvertUtils.getString(reflectHelper
				.getMethodValue("realpath"));
		String realpath = systemService.downFTPToSV(request.getSession()
				.getServletContext().getRealPath("/"),
				strRealPath.substring(0, strRealPath.lastIndexOf("/")),
				strRealPath.substring(strRealPath.lastIndexOf("/") + 1));

		if ("dwg".equals(extend)) {
//			String realpath = oConvertUtils.getString(reflectHelper
//					.getMethodValue("realpath"));
...
//			String swfpath = oConvertUtils.getString(reflectHelper
//					.getMethodValue("swfpath"));
			String swfpath = realpath.substring(0, realpath.lastIndexOf(".")) + ".swf";
	...
	public void viewFile(HttpServletRequest request,
		...
		String subname=request.getParameter("subclassname");
		if("c".equals(subname))
			subname="org.jeecgframework.web.cgform.entity.upload.CgUploadEntity";
		String subclassname = oConvertUtils.getString(
				subname,
		...
		String contentfield = oConvertUtils
				.getString(request.getParameter("contentfield"),
						uploadFile.getByteField());
		byte[] content = (byte[]) reflectHelper.getMethodValue(contentfield);
		String path = oConvertUtils.getString(reflectHelper
				.getMethodValue("realpath"));
		String extend = oConvertUtils.getString(reflectHelper
				.getMethodValue("extend"));
		String attachmenttitle = oConvertUtils.getString(reflectHelper
				.getMethodValue("attachmenttitle"));
	 ...
	 * @param response
	 */
	@RequestMapping(params = "viewFileftppath")
	public void viewFilebyFtpPath(HttpServletRequest request,
			HttpServletResponse response) {
		...
		String contentfield = oConvertUtils
				.getString(request.getParameter("contentfield"),
						uploadFile.getByteField());
		//byte[] content = (byte[]) reflectHelper.getMethodValue(contentfield);
		String path = request.getParameter("ftppath");
	 ...
	 * @throws FileNotFoundException
	 */
	@RequestMapping(params = "openJSPFile")
	public void openJSPFile(HttpServletRequest request,
			HttpServletResponse response){
		String fileid = request.getParameter("fileid");
		String subclassname = oConvertUtils.getString(
				request.getParameter("subclassname"),
		...
		String strRealPath = oConvertUtils.getString(reflectHelper
				.getMethodValue("realpath"));
		if(strRealPath != null && strRealPath.length()>0){
			String directory = strRealPath.substring(0, strRealPath.lastIndexOf("/"));
			String fileName = strRealPath.substring(strRealPath.lastIndexOf("/") + 1);
			systemService.downFTPToJsp(directory, fileName, response);
		}
	}
	...
	public void getAutoList(HttpServletRequest request,
			HttpServletResponse response, Autocomplete autocomplete) {
		...
		try {
			...
			response.getWriter().write(
					JSONHelper.listtojson(allFieldArr, allFieldArr.length,
							autoList));
	...
	public AjaxJson delObjFile(HttpServletRequest request) {
		...
		TSAttachment attachment = systemService.getEntity(TSAttachment.class,
				fileKey);
		String subclassname = attachment.getSubclassname(); // 子类类名
		Object objfile = systemService.getEntity(
				MyClassLoader.getClassByScn(subclassname), attachment.getId());// 子类对象
	...
	public ModelAndView objfileList(HttpServletRequest request) {
		...
		TSAttachment attachment = systemService.getEntity(TSAttachment.class,
				fileKey);
		String businessKey = oConvertUtils.getString(request
				.getParameter("businessKey"));// 业务主键
		String busentityName = oConvertUtils.getString(request
				.getParameter("busentityName"));// 业务主键
		String typename = oConvertUtils.getString(request
				.getParameter("typename"));// 类型
		String typecode = oConvertUtils.getString(request
				.getParameter("typecode"));// 类型typecode
		if (StringUtil.isNotEmpty(busentityName)
				&& StringUtil.isNotEmpty(businessKey)) {
			object = systemService.get(
					MyClassLoader.getClassByScn(busentityName), businessKey);
	...
	public void objfileGrid(HttpServletRequest request,
			HttpServletResponse response, DataGrid dataGrid) {
		String businessKey = oConvertUtils.getString(request
				.getParameter("businessKey"));
		String subclassname = oConvertUtils.getString(request
				.getParameter("subclassname"));// 子类类名
		...
		CriteriaQuery cq = new CriteriaQuery(
				MyClassLoader.getClassByScn(subclassname), dataGrid);
