...
public class SystemController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(params = "gisDemo")
	public ModelAndView gisDemo(HttpServletRequest request){
		String config = request.getParameter("config");
		request.setAttribute("config", config);
		return new ModelAndView("system/gis/gisdemo");
	}
	@RequestMapping(params = "updateGisxy")
	@ResponseBody
	public AjaxJson updateGisxy(HttpServletRequest request){
		AjaxJson json = new AjaxJson();
		String id = request.getParameter("id");
		String tablename = request.getParameter("tableName");
		String gisxy = request.getParameter("gisxy");
		if(StringUtil.isNotEmpty(id) && StringUtil.isNotEmpty(tablename) && StringUtil.isNotEmpty(gisxy)){
			String sql = "update "+tablename+" as t set t.gisxy = ? where t.id= ?";
			try {
				int i = systemService.executeSql(sql, gisxy,id);
				if(i > 0){
					json.setMsg("地理位置信息更新成功");
				}else{
					json.setMsg("地理位置信息更新失败");
					json.setSuccess(false);
				}
			} catch (Exception e) {
				json.setMsg("出错了"+e.toString());
				json.setSuccess(false);
			}
			
			
		}else{
			json.setMsg("参数不完整请检查");
			json.setSuccess(false);
		}
		return json;
	}
...
//        add-end--Author:zhangguoming  Date:20140929 for：数据字典修改
	...
	public AjaxJson saveDepart(TSDepart depart, HttpServletRequest request) {
		// 设置上级部门
		String pid = "";
		pid = request.getParameter("TSPDepart.id");
		if (null == pid || pid.equals("") || "null".equals(pid)) {
    ...
    /*SELECT * from (
    		select tsd1.id as id,tsd1.departname as name,tsd2.ID as pId,tsd2.departname as pname, 'true' as isParent from t_s_depart tsd1
    		LEFT JOIN t_s_depart tsd2 on tsd1.parentdepartid = tsd2.id  
    		UNION ALL
    		select tsbu.id as id ,tsbu.realname as name ,tsd.id as pId ,tsd.departname as pname ,'false' as isParent from t_s_base_user tsbu
    		LEFT JOIN t_s_depart tsd on tsbu.departid = tsd.id
    		) t
     ...
     * @param response
     */
	@RequestMapping(params = "getDepUserTree")
	public void getDepUserTree(HttpServletRequest request,HttpServletResponse response) {
 
        String id = request.getParameter("id");
	 
        StringBuffer sb = new StringBuffer("SELECT * from ( ")
        				.append(" select tsd1.id as id,tsd1.departname as name,tsd2.ID as pId,tsd2.departname as pname, 'true' as isParent from t_s_depart tsd1 ")
        				.append(" LEFT JOIN t_s_depart tsd2 on tsd1.parentdepartid = tsd2.id  ")
        				.append(" UNION ALL ")
        				.append(" select tsbu.id as id ,tsbu.realname as name ,tsd.id as pId ,tsd.departname as pname ,'false' as isParent from t_s_base_user tsbu ")
        				.append(" LEFT JOIN t_s_depart tsd on tsbu.departid = tsd.id")
        				.append(" ) t")  ;
        
		List<Map<String, Object>> list = null;
		if(StringUtil.isEmpty(id)){
			sb.append(" where pId is NULL or pid = '' ");
		}else{
			sb.append(" where pId = '" + id +"' ");
		}
		
	    list = systemService.findForJdbc(sb.toString());
	    
		String treeJson = JSONHelper.array2json(list);
     ...
     * @param response
     */
	@RequestMapping(params = "getDeptTree")
	public void getDeptTree(HttpServletRequest request,HttpServletResponse response) {
        ...
        StringBuffer org = new StringBuffer(" SELECT org_code FROM t_s_depart WHERE t_s_depart.ID in (SELECT org_id FROM t_s_user_org WHERE user_id = '"+userid+"')");
		List<Map<String, Object>> list2 = null;
		list2 = systemService.findForJdbc(org.toString());
		Map<String, Object> org2 = list2.get(0);
//		Set org3 = (Set) org2.keySet();
		Iterator iter = (Iterator)org2.values().iterator();
		String org3 = null;
		while(iter.hasNext()){
			org3 = (String) iter.next();
		}
//		String org4 = org3.substring(0, org3.length()-3);
		
        StringBuffer sb = new StringBuffer("SELECT * from ( ")
        				.append(" select tsd1.id as id,tsd1.departname as name,tsd2.ID as pId,tsd2.departname as pname,tsd1.org_code as orgcode,tsd1.org_code as org  from t_s_depart tsd1 ")
        				.append(" LEFT JOIN t_s_depart tsd2 on tsd1.parentdepartid = tsd2.id  ")
        				.append(" ) t WHERE t.org like '"+org3+"%'");
//        				.append(" SELECT org_id FROM t_s_user_org WHERE user_id = '" + userid + "')");
