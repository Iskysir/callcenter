...
public class TerritoryController extends BaseController {
	...
	@ResponseBody
	public AjaxJson getProvince(HttpServletRequest request){
		AjaxJson json = new AjaxJson();
		String sql = "SELECT  ID,TERRITORYNAME,TERRITORYCODE FROM t_s_territory " +
				"WHERE TERRITORYPARENTID = '1' ORDER BY TERRITORYSORT ASC";
	...
	public AjaxJson getCity(HttpServletRequest request){
		...
		String sql = "SELECT ID,TERRITORYNAME,TERRITORYCODE FROM t_s_territory where TERRITORYPARENTID in( SELECT  ID FROM t_s_territory " +
				"WHERE TERRITORYCODE = ?) ORDER BY TERRITORYSORT ASC";
		/*CriteriaQuery cq = new CriteriaQuery(TSTerritory.class);
		cq.eq("TSTerritory.id","1");//这个是全国最高级
		cq.addOrder("territorySort", SortDirection.asc);
		cq.add();
		List<TSTerritory> list = systemService.getListByCriteriaQuery(cq,false);*/
		List list = systemService.findForJdbc(sql,code);
		json.setObj(list);
		return  json;
	}
	/**
