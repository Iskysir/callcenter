...
var config =eval('('+urlConfigString+')');
    ...
    {
        if("${config_id}"=="hl_bucusname_info"||"${config_id}"=="V_BiActCardAccTb"||"${config_id}"=="V_BiSlpCar")
        {
            return "";
        }
        else
        {
            "cgReportController.do?datagrid&configId=${config_id}";
        }
    };
$('#${config_id}List').datagrid(
	...
	function ${config_id}Listsearch(){
 		var queryParams=$('#${config_id}List').datagrid('options').queryParams;$('#${config_id}Listtb').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});$('#${config_id}List').datagrid({url:'cgReportController.do?datagrid&configId=${config_id}',pageNumber:1});
 	}
