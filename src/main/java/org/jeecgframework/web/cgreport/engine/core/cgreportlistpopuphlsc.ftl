...
$('#${config_id}List').datagrid(
	...
	idField: 'id',
	/*title: '${config_name}',*/
	url:'cgReportControllerHlSc.do?datagrid&configId=${config_id}',
        queryParams: {
            create_by: '8004'
        },
	fit:true,
	...
	columns:[
		[			<#if (config_fieldList?size>0)>
					...
					</#if>
            {
                ...
                var href = '';
                href += "[<a href='#' onclick=deletesc('" + rec.account + "')>";
                href += "取消收藏</a>]";
				return href;
            }
            }
...
function  deletesc(account){
	var url = "hlVisitItemController.do?deletesc&usercode="+account;
...
function reloadTable(){
	...
	function ${config_id}Listsearch(){
 		var queryParams=$('#${config_id}List').datagrid('options').queryParams;$('#${config_id}Listtb').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});$('#${config_id}List').datagrid({url:'cgReportControllerHlSc.do?datagrid&configId=${config_id}',pageNumber:1});
 	}
	function dosearch(params){var jsonparams=$.parseJSON(params);$('#${config_id}List').datagrid({url:'cgReportControllerHlSc.do?datagrid&configId=${config_id},',queryParams:jsonparams});}
