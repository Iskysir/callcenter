 ...
 */
@Controller
@RequestMapping("/cgReportControllerHlSc")
public class CgReportControllerHlSc extends BaseController {
	@Autowired
	...
	public void popup(String id, HttpServletRequest request,
		...
		cgReportMap.put("config", config);
		String html = viewEngine.parseTemplate("/org/jeecgframework/web/cgreport/engine/core/cgreportlistpopuphlsc.ftl", cgReportMap);
		try {
			response.setContentType("text/html");
			response.setHeader("Cache-Control", "no-store");
			PrintWriter writer = response.getWriter();
	...
	private void loadVars(Map<String, Object> cgReportMap) {
		...
		cgReportMap.put("pagination", mainM.get("pagination"));
	}
	/**
	 * 处理数据字典
	 * @param result 查询的结果集
				...
				for(Map r:result){
					...
					for(Map m:dicDatas){
						...
						if(value.equalsIgnoreCase(typecode)){
							...
							r.put(bean.get(CgReportConstant.ITEM_FIELDNAME) + "_codevalue",typecode);
						}
					}
				}
			}
	...
	public void datagrid(String configId,String page,String field,String rows, HttpServletRequest request,
		...
		querySql+=" where create_by='"+user.getUserName()+"' ORDER BY n.name asc";
