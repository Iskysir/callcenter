...
public class ConfigServiceImpl implements ConfigServiceI {
	...
	public Map<String, Object> queryConfigsByCode(String code,String jversion) {
		//step.1 要返回的配置数据
		Map<String, Object> configs = new HashMap<String,Object>();
		//step.2 获取动态表配置
		CgFormHeadEntity tableEntity = null;
		try{
			tableEntity = tablePropertyService.findByProperty(CgFormHeadEntity.class, "code", code).get(0);
			loadConfigs(configs,tableEntity);
		}catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("没有找到该动态列表");
	...
	private void loadConfigs(Map<String, Object> configs, CgFormHeadEntity tableEntity) {
		...
		configs.put(CgAutoListConstant.CONFIG_ID, tableEntity.getCode());
		...
		if(buttons != null){
			for (CgformButtonEntity cb : buttons) {
				if("link".equals(cb.getButtonStyle())){
					link_buttons.add(cb);
				}
			}
		}
		configs.put("config_link_buttons",link_buttons);
