...
public class DataBaseServiceImpl extends CommonServiceImpl implements DataBaseService{
	...
	public int insertTable(String tableName, Map<String, Object> data) {
		CgFormHeadEntity cgFormHeadEntity = cgFormFieldService.getCgFormHeadByCode(tableName);
		if(cgFormHeadEntity==null)
		{
			cgFormHeadEntity = cgFormFieldService.getCgFormHeadByTableName(tableName);
		}
		//系统上下文变量赋值
		...
		for (Entry<String, Object> entry : data.entrySet()) {
			// 判断key是否为表配置的属性
			if(isContainsFieled(cgFormHeadEntity.getTableName(),entry.getKey())){
		...
		String sql = "INSERT INTO " + cgFormHeadEntity.getTableName() + " (" + insertKey + ") VALUES (" + insertValue + ")";
		Object key = null;
		try {
			//类型转换冲突
			try {
	...
	private Map<String, Object> dataAdapter(String tableName,Map<String, Object> data) {
		//step.1 获取表单的字段配置
		Map<String, CgFormFieldEntity> fieldConfigs =cgFormFieldService.getAllCgFormFieldByCode(tableName);
		//step.2 迭代将要持久化的数据
				...
				if("date".equalsIgnoreCase(type)){
					...
					try {
						...
						if("datetime".equalsIgnoreCase(dateType)){
							...
							if(beforeV!=null&&beforeV.toString().length()>10)
							{
								newV = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(String.valueOf(beforeV));
							}
							else
	...
	public int updateTable(String tableName, Object id, Map<String, Object> data) {
		...
		dataAdapter(cgFormHeadEntity.getTableName(),data);
		String comma = "";
		
		
		List<CgFormFieldEntity> fieldList=cgFormHeadEntity.getColumns();
		for(CgFormFieldEntity fieldEntity:fieldList)
		{
			if(fieldEntity.getShowType().equals("checkbox")||fieldEntity.getShowType().equals("radio"))
			{
				if(data.containsKey(fieldEntity.getFieldName()))
				{
					
				}
				else
				{
					data.put(fieldEntity.getFieldName(), null);
				}
				
			}
		}
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("update ").append(cgFormHeadEntity.getTableName()).append(" set ");
		
		if (!"".equals(data.get("version_num")) && !"null".equals(data.get("version_num")) && null != data.get("version_num")) {

		} else {
			data.put("version_num", 0);
		}
		...
		Map<String, Object> map = this.findOneForJdbc(sb.toString());
		int num2 = 0;
		if (map != null && map.size() > 0) {
			if (map.containsKey("version_num")) {
				if (null != map.get("version_num") && !"".equals(map.get("version_num") + "") && !"null".equals(map.get("version_num") + "")) {
					num2 = Integer.valueOf(map.get("version_num") + "");
				}

				int num1 = Integer.valueOf(data.get("version_num") + "");
				if (num1 == num2) {
					data.put("version_num", num1 + 1);
				} else {
		...
		if(cgformButtonSqlVo!=null){
			...
			if(StringUtils.isNotEmpty(sqlPlugin)){
				...
				for(String sql:sqls){
					...
					if(sql.toLowerCase().indexOf(CgAutoListConstant.SQL_INSERT)!=-1
							||sql.toLowerCase().indexOf(CgAutoListConstant.SQL_UPDATE)!=-1 || sql.toLowerCase().indexOf(CgAutoListConstant.SQL_DELETE) != -1){
						//执行sql
...
//            sql = sql.replace("${" + key + "}", "'"+String.valueOf(params.get(key))+"'");
	...
	public Map<String, Object> insertTableMore(Map<String, List<Map<String, Object>>> mapMore, String mainTableName) throws BusinessException {
		...
		while(it.hasNext()){
	        ...
	        for(Map<String, Object> fieldMap:ov){
	        	//处理关联键
	        	List<Map<String, Object>> fkFieldList =  getFKFieldByCode(mainTableName, ok);
	...
	public boolean updateTableMore(Map<String, List<Map<String, Object>>> mapMore, String mainTableName) throws BusinessException {
		...
		mapMore = CommUtils.attributeMapFilter(mapMore,filterMainTable);
		Iterator it=mapMore.entrySet().iterator();
		//解决附件表数据剩余一条，不能删除问题开始
		cgFormFieldService.deleteSubTalble(mainTableName, mainTableId, mapMore);
		//解决附件表数据剩余一条，不能删除问题结束
		while(it.hasNext()){
	        Map.Entry entry=(Map.Entry)it.next();
	        String ok=(String)entry.getKey();
	        List<Map<String, Object>> ov=(List<Map<String, Object>>)entry.getValue();
	        //处理关联键
        	List<Map<String, Object>> fkFieldList =  getFKFieldByCode(mainTableName, ok);
        	Map<String,Object> oneFK=fkFieldList!=null&&fkFieldList.size()>0?fkFieldList.get(0):null;
        	//根据主表id获取附表信息
        	String mainTableNameTemp=mainTableName,subTableTemp=ok;
        	if(oneFK!=null)
        	{
        		mainTableNameTemp=(String)oneFK.get("main_table");
        		subTableTemp=(String)oneFK.get("table_name");
        	}
        	Map<Object,Map<String, Object>> subTableDateMap = getSubTableData(fkFieldList, mainTableNameTemp, subTableTemp, mainTableId);
	        for(Map<String, Object> fieldMap:ov){
	...
	private List<Map<String, Object>> getFKFieldByCode(String mainTableName, String subTableName) {
		StringBuilder sql1 = new StringBuilder("");
		sql1.append("select f.*,h.table_name from cgform_field f ,cgform_head h");
		sql1.append(" where f.table_id = h.id ");
	...
	public Object getPkValue(String tableName) {
		Object pkValue = null;
		CgFormHeadEntity  cghead = cgFormFieldService.getCgFormHeadByCode(tableName);
		if(cghead==null)
		{
			 cghead = cgFormFieldService.getCgFormHeadByTableName(tableName);
		}
		String dbType = DBTypeUtil.getDBType();
	...
	private void fillUpdateSysVar(Map<String, Object> data) {
		if(data.containsKey(DataBaseConstant.UPDATE_DATE_TABLE)){
			data.put(DataBaseConstant.UPDATE_DATE_TABLE, DateUtils.formatTime());
		}
	...
	private void fillInsertSysVar(Map<String, Object> data) {
		if(data.containsKey(DataBaseConstant.CREATE_DATE_TABLE)){
			data.put(DataBaseConstant.CREATE_DATE_TABLE, DateUtils.formatTime());
		}
		...
		if(data.containsKey(DataBaseConstant.SYS_ORG_CODE_TABLE)&&StringUtils.isEmpty(String.valueOf(data.get(DataBaseConstant.SYS_ORG_CODE_TABLE)))){
		...
		if(data.containsKey(DataBaseConstant.CREATE_DEPARTMENT_TABLE)){
			data.put(DataBaseConstant.CREATE_DEPARTMENT_TABLE, ResourceUtil.getUserSystemData(DataBaseConstant.CREATE_DEPARTMENT));
		}
