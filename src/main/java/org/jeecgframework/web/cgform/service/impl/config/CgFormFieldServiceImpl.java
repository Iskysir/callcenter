...
public class CgFormFieldServiceImpl extends CommonServiceImpl implements
	...
	public synchronized void updateTable(CgFormHeadEntity t, String sign) {
		...
		for (int i = 0; i < t.getColumns().size(); i++) {
			...
			column.setTable(t);
			// 设置checkbox的值
			PublicUtil.judgeCheckboxValue(column,"isNull,isShow,isShowList,isQuery,isKey,mainQuery");
	...
	public Boolean judgeTableIsExitByCode(String code)
	{
		boolean result=false;
		CgFormHeadEntity table = this.findUniqueByProperty(CgFormHeadEntity.class, "code",code);
		result=judgeTableIsExit(table.getTableName());
		return result;
	}
	...
	public Map<String, CgFormFieldEntity> getAllCgFormFieldByCode(
			String code) {
		StringBuilder hql = new StringBuilder("");
		hql.append("from CgFormFieldEntity f");
		hql.append(" where f.table.code=? ");
		List<CgFormFieldEntity> list = this.findHql(hql.toString(), code);
		Map<String, CgFormFieldEntity> map = new HashMap<String, CgFormFieldEntity>();
		if (list != null && list.size() > 0) {
			for (CgFormFieldEntity po : list) {
				map.put(po.getFieldName(), po);
	...
	public CgFormHeadEntity getCgFormHeadByCode(String tableName) {
		StringBuilder hql = new StringBuilder("");
		hql.append("from CgFormHeadEntity f");
		hql.append(" where f.code=? ");
		List<CgFormHeadEntity> list = this.findHql(hql.toString(), tableName);
		if (list != null && list.size() > 0) {
			return list.get(0);
		}
	...
	public List<Map<String, Object>> getSubTableData(String mainTableName,
		...
		sql3.append("select * from cgform_field where field_name = 'sort_no' and table_id =? ");
		String subId="";
		...
		if (list != null && list.size() > 0) {
			for (Map<String, Object> map : list) {
				...
				subId = map.get("table_id")+"";
			}
		}
		sql2.append(" and main.id= ? ");
		//排序，若有sort_no字段，按此字段排序
		if(!"".equals(subId) && null != subId){
			Map<String, Object> map3 = this.findOneForJdbc(sql3.toString(),subId);
			if(null != map3 && map3.size() >0){
				sql2.append(" order by sub.").append(map3.get("field_name"));
			}
	...
	public void deleteSubTableDataByWjId(String mainTableName,
			String subTableName, Object mainTableId){
		StringBuilder sql1 = new StringBuilder("");
		sql1.append("select f.* from cgform_field f ,cgform_head h");
		sql1.append(" where f.table_id = h.id ");
		...
		if (list != null && list.size() > 0) {
			for (Map<String, Object> map : list) {
				String filedName = (String)map.get("field_name");
				if(StringUtils.isNotEmpty(filedName)){
					StringBuilder sql = new StringBuilder("");
					sql.append(" delete from ").append(subTableName).append(" where ").append(filedName).append(" = ?");	
					this.executeSql(sql.toString(), mainTableId);
				}
	...
	public void deleteSubTableDataByCode(String mainTableName,
			String subTableName, Object mainTableId){
		StringBuilder sql1 = new StringBuilder("");
		sql1.append("select f.*,h.table_name from cgform_field f ,cgform_head h");
		sql1.append(" where f.table_id = h.id ");
		sql1.append(" and h.code=? ");
		sql1.append(" and f.main_tablecode=? ");
		List<Map<String, Object>> list = this.findForJdbc(sql1.toString(),
				subTableName, mainTableName);
		if (list != null && list.size() > 0) {
			for (Map<String, Object> map : list) {
				String filedName = (String)map.get("field_name");
				String subTableNameTemp=(String)map.get("table_name");
				if(StringUtils.isNotEmpty(filedName)){
					StringBuilder sql = new StringBuilder("");
					sql.append(" delete from ").append(subTableNameTemp).append(" where ").append(filedName).append(" = ?");	
					this.executeSql(sql.toString(), mainTableId);
				}
	...
	public void deleteSubTalble(String mainTableName,Object mainTableId,Map<String, List<Map<String, Object>>> mapMore){
		String version = this.getCgFormVersionByTableName(mainTableName);
    	CgFormHeadEntity cgFormHeadEntity = this.getCgFormHeadByCode(mainTableName,version);
    	String subTableStr = cgFormHeadEntity.getSubTableStr();	
    	if(StringUtils.isNotEmpty(cgFormHeadEntity.getSubCode()))
    	{
    		subTableStr=cgFormHeadEntity.getSubCode();
    	}
    	...
    	for(Map.Entry<String, List<Map<String, Object>>> entry:mapMore.entrySet()){    
    		List<Map<String, Object>> list  = entry.getValue();
    		String ok = entry.getKey();
    		 if(subMap.containsKey(ok)){
    			 subMap.remove(ok);
    		 }
	...
	public boolean appendSubTableStr4Main(CgFormHeadEntity entity) {
		...
		for (CgFormFieldEntity fieldE : columns) {
			...
			String mainTCode=fieldE.getMainTableCode();//zxy code
			if (!StringUtil.isEmpty(mainT) && !StringUtil.isEmpty(mainF)) {
				CgFormHeadEntity mainE =null;
				if(StringUtil.isNotEmpty(mainTCode))//zxy code
					mainE=this.getCgFormHeadByCode(mainTCode);//zxy code
				else//zxy code
					mainE=this.getCgFormHeadByTableName(mainT);
				if (mainE == null) {
					continue;
				}
				// step.4 追加处理主表的附表串
				...
				//zxy code  begin
				// step.4 追加处理主表的附表串
				String subCode= String.valueOf(mainE.getSubCode() == null ? "" : mainE.getSubCode());
				// step.5 判断是否已经存在于附表串
				if (!subCode.contains(thisSubCode)) {
					// step.6 追加到附表串
					...
					mainE.setSubCode(subCode);
					logger.info("--主表编号" + mainE.getCode() + "的附表Code串："
							+ mainE.getSubCode());
				}
				//zxy code  end
				// step.7 更新主表的表配置
	...
	public boolean removeSubTableStr4Main(CgFormHeadEntity entity) {
		...
		// step.1 获取本表的名称
		String thisSubTable = entity.getTableName();
		String thisSubCode=entity.getCode();//zxy code add
		List<CgFormFieldEntity> columns = entity.getColumns();
		// step.2 扫描字段配置，循环处理填有主表以及主表字段的条目
		for (CgFormFieldEntity fieldE : columns) {
			String mainT = fieldE.getMainTable();
			...
			if (!StringUtil.isEmpty(mainT) && !StringUtil.isEmpty(mainF)) {
				...
				//zxy 2015年7月15日16:37:00 begin
				// step.4 追加处理主表Code的附表串
				String subTableCode = String.valueOf(mainE.getSubCode() == null ? "" : mainE.getSubCode());
				// step.5 判断是否已经存在于附表Code串
				if (subTableCode.contains(thisSubCode)) {
					// step.6 剔除主表的附表串
					if (subTableCode.contains(thisSubCode + ",")) {
						subTableCode = subTableCode.replace(thisSubCode + ",",
								"");
					} else if(subTableCode.contains(","+thisSubCode)){
						subTableCode = subTableCode.replace("," + thisSubCode,
								"");
					} else{
						subTableCode = subTableCode.replace(thisSubCode,"");
					}
					mainE.setSubCode(subTableCode);
					logger.info("--主表编号" + mainE.getCode() + "的附表Code串："
							+ mainE.getSubCode());
					// step.7 更新主表的表配置,不用更新,因为hibernate是读取的缓存,所以和那边操作的对象是一个的
					//this.updateTable(mainE, null);
				}
				//zxy 2015年7月15日16:37:00 end
	...
	public void sortSubTableStr(CgFormHeadEntity entity) {
		...
		for (CgFormFieldEntity fieldE : columns) {
			String mainT = fieldE.getMainTable();
			String mainF = fieldE.getMainField();
			String mainTCode= fieldE.getMainTableCode();
			if (!StringUtil.isEmpty(mainT) && !StringUtil.isEmpty(mainF)) {
				CgFormHeadEntity mainE = null;
				if(StringUtil.isNotEmpty(mainTCode))//zxy code 
				{//zxy code 
					mainE = this.getCgFormHeadByTableName(mainTCode);//zxy code 
				}//zxy code 
				else
				{
					mainE = this.getCgFormHeadByTableName(mainT);
		...
		String subTableStr = null;
		String subCode=main.getSubCode();
		if(StringUtils.isNotEmpty(subCode))//zxy code begin
		{
			subTableStr = subCode;//zxy code begin
		}
		else
		{
			subTableStr = main.getSubTableStr();//zxy code begin
		}
		if(StringUtils.isNotEmpty(subTableStr)){
			String [] subTables = subTableStr.split(",");
			...
			for(String subTable : subTables){
				CgFormHeadEntity sub = this.getCgFormHeadByCode(subTable);
				subList.add(sub);
			}
	...
	public Map<String, Object> getFtlFormConfig(String tableName, String version) {
		...
		//zxy 2015年7月10日 begin
		CgFormHeadEntity head = this.getCgFormHeadByCode(tableName,
				version);
		//zxy 2015年7月10日 end
		data.put("head", head);
		if (head.getJformType() == CgAutoListConstant.JFORM_TYPE_MAIN_TALBE) {
			...
			if (StringUtils.isNotEmpty(subTableStr)) {
				String[] subTables = subTableStr.split(",");
				String[] subTableNames=head.getSubTableStr().split(",");
				...
				for (String subTable : subTables) {
					...
					CgFormHeadEntity subhead =null;
					if(StringUtils.isNotEmpty(subCode))//zxy code 处理
					{
						subhead = this.getCgFormHeadByCode(subTable);
					}
					else
					{
						subhead = this.getCgFormHeadByTableName(subTable);
					}
		...
		data.put("tableName", head.getCode());
		List<Map<String, Object>> fieldList = null;
		if (head.getJformType() == CgAutoListConstant.JFORM_TYPE_MAIN_TALBE) {
			// 查询主表或单表表单配置
			fieldList = this.getCgFormFieldByTableName(tableName);
		} else {
			Map<String, Object> cgformFtlEntity = cgformFtlService.getCgformFtlByCode(tableName);
		...
		if (fieldList != null) {
			...
			List<Map<String, Object>> typeTitleList = new ArrayList<Map<String, Object>>();
			for (Map<String, Object> map : fieldList) {
				...
				if(("popup".equals((String) map.get("show_type"))||"idcardread".equals((String) map.get("show_type"))))
				{
					popupList.add(map);
				}else if("typetitle".equals((String) map.get("show_type"))){
					typeTitleList.add(map);
				}
				if("date".equals((String) map.get("show_type"))||"datetime".equals((String) map.get("show_type"))||"Date".equals((String) map.get("type")))
				{
					dateList.add(map);
				}
			}
			...
			if(hiddenFieldList != null){
				for (Map<String, Object> map : hiddenFieldList) {
					if("date".equals((String) map.get("show_type"))||"datetime".equals((String) map.get("show_type"))||"Date".equals((String) map.get("type")))
					{
						dateList.add(map);
					}
			...
			data.put("popupList", popupList);
			data.put("dateList", dateList);
			data.put("typeTitleList", typeTitleList);
		}
		// js增强
		String jsCode = "";
	...
	@Ehcache
	public CgFormHeadEntity getCgFormHeadByCode(String code,
			String version) {
		StringBuilder hql = new StringBuilder("");
		hql.append("from CgFormHeadEntity f");
		hql.append(" where f.code=? ");
		List<CgFormHeadEntity> list = this.findHql(hql.toString(), code);
		if (list != null && list.size() > 0) {
			return list.get(0);
		}
		return null;
	...
	public ChrhcFormInterceptor getInterceptorByTableName(String tableName) {
		ChrhcFormInterceptor interceptor = null;
		
		//String version = this.getCgFormVersionByTableName(tableName);
		CgFormHeadEntity head = this.getCgFormHeadByCode(tableName);
		if(StringUtil.isNotEmpty(head.getInterClass())){
			try {
				interceptor = (ChrhcFormInterceptor) Class.forName(head.getInterClass()).newInstance();
