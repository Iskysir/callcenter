	...
	private static HSSFWorkbook exportExcelInUserModel2File(String title,
		...
		try {
			...
			while (it.hasNext()) {
				...
				// 输入需要显示的字段信息
				if (type.getIsShow().equals("Y") || type.getIsShowList().equals("Y")) {
				...
				while (it.hasNext()) {
					...
					if (type.getIsShow().equals("Y") || type.getIsShowList().equals("Y")) {
						...
						if(datalist.get(i).get(type.getFieldName()) != null){
							...
							if("double".equals(type.getType())){
								try{
									BigDecimal bd = new BigDecimal(datalist.get(i).get(type.getFieldName()).toString());  
									text = new HSSFRichTextString(bd.toPlainString());
								}catch(Exception e) {
									text = new HSSFRichTextString(datalist.get(i).get(type.getFieldName()).toString());
								}
							}else {
								text = new HSSFRichTextString(datalist.get(i).get(type.getFieldName()).toString());
							}
