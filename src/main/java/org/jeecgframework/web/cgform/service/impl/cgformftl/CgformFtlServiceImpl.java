...
//    update-start--Author:zhangguoming  Date:20140922 for：根据ftlVersion动态读取模板
    ...
    public Map<String, Object> getCgformFtlByCode(String code, String ftlVersion) {
        StringBuilder sql = new StringBuilder("");
        sql.append("select ftl.* from cgform_ftl ftl,cgform_head head");
        sql.append(" where ftl.cgform_id=head.id");
        sql.append(" and ftl.ftl_version=? ");
        sql.append(" and head.code=? ");
        List<Map<String,Object>> list = this.findForJdbc(sql.toString(), ftlVersion, code);
        if(list!=null&&list.size()>0){
            return list.get(0);
        }
        return null;
...
//    update-end--Author:zhangguoming  Date:20140922 for：根据ftlVersion动态读取模板
	...
	public Map<String,Object> getCgformFtlByCode(String code) {
		StringBuilder sql = new StringBuilder("");
		sql.append("select ftl.* from cgform_ftl ftl,cgform_head head");
		sql.append(" where ftl.cgform_id=head.id");
		sql.append(" and ftl.ftl_status='1'");
