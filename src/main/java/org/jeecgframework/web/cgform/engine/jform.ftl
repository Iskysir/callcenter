			   ...
			   <#list columns as po>
				...
				<#else>
					...
					<#if po.show_type=='typetitle'>
						...
						<#if ((po_index-typetitle_index-1)%2==0 && typetitle_index != 0)>
							<td align="right">
					...
					<td class="value">
						...
						<#elseif po.show_type=='popupgis'>
							<input id="${po.field_name}" name="${po.field_name}"  type="text"
							       style="width: 150px" class="searchbox-inputtext" 
							       onClick="inputClickGis(this,'${po.dict_text?if_exists?html}','${po.dict_table?if_exists?html}');" 
							       value="${data['${tableName}']['${po.field_name}']?if_exists?html}"
					               <#if po.operationCodesReadOnly?if_exists> readonly = "readonly"</#if>
						       ...
						       <#if po.field_valid_type?if_exists?html != ''>
					               ...
					               <#if po.is_null != 'Y'>datatype="*"</#if>
					                 ...
					                 <a href="#" class="easyui-linkbutton" plain="true" icon="icon-redo" onClick="clearGis('${po.field_name}')">清空</a>
