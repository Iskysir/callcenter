...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		String dataRuleParam=request.getParameter("dataRule");
		if(client == null){ 
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser()!=null ) {
				...
				if((!true) && !client.getUser().getUserName().equals("admin")){
				...
				if(requestPath.equals("chrhcAutoListController.do?datagrid")) {
					//requestPath += "&configId=" +  request.getParameter("configId");
				}
				if(requestPath.equals("chrhcAutoListController.do?datagrid2")) {
					requestPath += "&configId=" +  request.getParameter("configId");
				}
				if(requestPath.equals("chrhcAutoListController.do?list")) {
				...
				if(!oConvertUtils.isEmpty(functionId)){
					...
					if(allOperation.size()>0){
						...
						String hasOperSql="SELECT to_char(operation) FROM t_s_role_function fun, t_s_role_user role WHERE  " +
							"fun.functionid='"+functionId+"' AND to_char(fun.operation)!=''  AND fun.roleid=role.roleid AND role.userid='"+client.getUser().getId()+"' ";
