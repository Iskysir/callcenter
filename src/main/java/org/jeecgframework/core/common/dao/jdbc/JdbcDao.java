...
public class JdbcDao extends SimpleJdbcTemplate{
	...
	//161227oracle分页排序混乱问题修改
	public static final String ORACLE_SQL_YT = "select * from (select row_.*,rownum rownum_ from ({0} order by rownum) row_ where rownum <= {1}) where rownum_>{2}"; //oracle
	...
	/**
	 * 使用指定的检索标准检索数据并分页返回数据(oracle版本使用分页排序)
	 */
	public List<Map<String, Object>> findForJdbc(String sql, int page, int rows,String type) {
		//封装分页SQL
		sql = jeecgCreatePageSqloracle(sql,page,rows);
	...
	/**
	 * 按照数据库类型，封装SQL oracle分页排序使用
	 */
	public static String jeecgCreatePageSqloracle(String sql, int page, int rows){
		...
		if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_MYSQL)!=-1){
			...
			if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_ORACLE)!=-1) {
				sql = MessageFormat.format(ORACLE_SQL_YT, sqlParam);
