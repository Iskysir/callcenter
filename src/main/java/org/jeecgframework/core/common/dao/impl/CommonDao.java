...
import com.chrhc.common.MD5Util;
...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao,
	...
	public void pwdInit(TSUser user, String newPwd) {
		...
		if (null != users && users.size() > 0) {
			user = users.get(0);
			String pwd = PasswordUtil.encrypt(user.getUserName(), newPwd,
	...
	public String getUserRole(TSUser user) {
		String userRole = "";
		List<TSRoleUser> sRoleUser = findByProperty(TSRoleUser.class,
	...
	// uploadFile.getMultipartRequest();
	...
	// String realPath =
	// uploadFile.getMultipartRequest().getSession().getServletContext().getRealPath("/")
	// + "/" + path;// 文件的硬盘真实路径
	...
	// reflectHelper.setMethodValue(uploadFile.getTitleField(),
	// fileprefixName);// 动态调用set方法给文件对象标题赋值
	// }
	...
	// // 设置文件数据库的物理路径
	// reflectHelper.setMethodValue(uploadFile.getRealPath(), path +
	// myfilename);
	// }
	// saveOrUpdate(object);
	// // 文件拷贝到指定硬盘目录
	...
	// // // 转SWF
	// // reflectHelper.setMethodValue(uploadFile.getSwfpath(), path + swfName +
	// ".swf");
	...
	public Object uploadFile(UploadFile uploadFile) {
		...
		if (uploadFile.getFileKey() != null) {
			...
			try {
				...
				// 设置文件上传路径+表明
				String formName = oConvertUtils.getString(reflectHelper
						.getMethodValue(uploadFile.getCGFORMNAME()));
				if (formName != null) {
					path = path + formName + "/";
				}

				// 获取FTP
				ftpClient = ftp.getFtpClient(path);
				...
				for (Map.Entry<String, MultipartFile> entity : fileMap
					...
					if (uploadFile.getSubClassName() != null) {
						// 设置文件数据库的子类名称全路径
						reflectHelper.setMethodValue(uploadFile.getSubClassName(),
								MyClassLoader.getPackPath(object));
					}
					saveOrUpdate(object);

					ftp.upload(mf.getInputStream(), path, myfilename, fileName,
							ftpClient);
				}
			} catch (Exception e1) {
			} finally {
	...
	@SuppressWarnings("unchecked")
	public Object uploadFileLocal(UploadFile uploadFile) {
		Object object = uploadFile.getObject();
		if(uploadFile.getFileKey()!=null)
		{
			updateEntitie(object);
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
	...
	// String((uploadFile.getTitleField() + "." +
	...
	public HttpServletResponse viewOrDownloadFile(UploadFile uploadFile) {
		...
		uploadFile.getResponse().setCharacterEncoding("UTF-8");
		HttpServletResponse response = uploadFile.getResponse();
		String downLoadPath = uploadFile.getRealPath();
		String directory = downLoadPath.substring(0,
		...
		try {
			...
			FtpUtils ftp = new FtpUtils();
			String ftpname = downLoadPath.substring(downLoadPath
					.lastIndexOf("/") + 1);
			ftp.download(response.getOutputStream(), directory, ftpname);
		} catch (IOException e) {
			e.printStackTrace();
		} catch (Exception e) {
	 ...
	 * @return 下载文件地址和文件名
	 */
	public String downFTPToSV(UploadFile uploadFile)
			throws FileNotFoundException {
		// FTP文件路径
		String downLoadPath = uploadFile.getRealPath();
		String directory = downLoadPath.substring(0,
				downLoadPath.lastIndexOf("/"));
		String ftpname = downLoadPath
				.substring(downLoadPath.lastIndexOf("/") + 1);
		// Server文件路径
		String strSvPath = "";
		String realPath = "FtpToSv/" + directory + "/";
		if (uploadFile.getMultipartRequest() != null) {
			realPath = uploadFile.getMultipartRequest().getSession()
					.getServletContext().getRealPath("/")
					+ realPath;
		} else {
			realPath = uploadFile.getRequest().getSession().getServletContext()
					.getRealPath("/")
					+ realPath;
		}
		strSvPath = realPath + ftpname;
		...
		FileUtils.delete(strSvPath);
		file = new File(strSvPath);
		FileOutputStream fos = new FileOutputStream(file);
		try {
			// 下载FTP文件至Server
			FtpUtils ftp = new FtpUtils();
			ftp.download(fos, directory, ftpname);

			// 转SWF
			String extend = FileUtils.getExtend(ftpname);
			if ("dwg".equals(extend)) {

			} else if (FileUtils.isPicture(extend)) {

			} else {
				SwfToolsUtil.convert2SWF(strSvPath);
			}
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * @throws FileNotFoundException
	 */
	public String downFTPToSV(String svRealPath, String directory,
			String fileName) throws FileNotFoundException {
		// Server文件路径
		String strSvPath = "";
		String realPath = "FtpToSv/" + directory + "/";
		strSvPath = realPath + fileName;

		// 创建文件
		File file = new File(svRealPath + realPath);
		if (!file.exists()) {
			file.mkdirs();// 创建根目录
		}
		FileUtils.delete(svRealPath + strSvPath);
		file = new File(svRealPath + strSvPath);
		FileOutputStream fos = new FileOutputStream(file);
		try {
			// 下载FTP文件至Server
			FtpUtils ftp = new FtpUtils();
			ftp.download(fos, directory, fileName);
			...
			if ("dwg".equals(extend)) {
				...
				SwfToolsUtil.convert2SWF(svRealPath + strSvPath);
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void downFTPToJsp(String directory, String fileName,
		...
		try {
			...
			FtpUtils ftp = new FtpUtils();
			response.setContentType("image/jpeg");
			ftp.download(response.getOutputStream(), directory, fileName);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public HttpServletResponse createXml(ImportFile importFile) {
		...
		try {
			...
			for (Object t : objList) {
				Element childElement = rElement.addElement(importFile
						.getEntityName());
				for (int i = 0; i < fields.length; i++) {
					String fieldName = fields[i];
					// 第一为实体的主键
					if (i == 0) {
						childElement.addAttribute(fieldName, String
								.valueOf(TagUtil
										.fieldNametoValues(fieldName, t)));
					} else {
						Element name = childElement.addElement(fieldName);
						name.setText(String.valueOf(TagUtil.fieldNametoValues(
								fieldName, t)));
					}
				}
				...
				for (Iterator j = employee.elementIterator(); j.hasNext();) {
					...
					for (int k = 0; k < fields.length; k++) {
						if (node.getName().equals(fields[k].getName())) {
							...
							Method setMethod = entityClass.getMethod(setName,
									new Class[] { fields[k].getType() });
							String type = TagUtil.getColumnType(fieldName,
									fields);
							if (type.equals("int")) {
								setMethod.invoke(obj1,
										new Integer(node.getText()));
							} else if (type.equals("string")) {
								setMethod.invoke(obj1, node.getText()
										.toString());
							} else if (type.equals("short")) {
								setMethod.invoke(obj1,
										new Short(node.getText()));
							} else if (type.equals("double")) {
								setMethod.invoke(obj1,
										new Double(node.getText()));
							} else if (type.equals("Timestamp")) {
								setMethod.invoke(
										obj1,
										new Timestamp(DateUtils.str2Date(
												node.getText(),
	...
	public ComboTree tree(TSDepart depart, boolean recursive) {
		...
		List<TSDepart> departsList = findByProperty(TSDepart.class,
				"TSPDepart.id", depart.getId());
	 ...
	 * @return
	 */
	private ComboTree comboTree(Object obj, ComboTreeModel comboTreeModel,
			List in, boolean recursive) {
		ComboTree tree = new ComboTree();
		Map<String, Object> attributes = new HashMap<String, Object>();
		ReflectHelper reflectHelper = new ReflectHelper(obj);
		String id = oConvertUtils.getString(reflectHelper
				.getMethodValue(comboTreeModel.getIdField()));
		tree.setId(id);
		tree.setText(oConvertUtils.getString(reflectHelper
				.getMethodValue(comboTreeModel.getTextField())));
		if (comboTreeModel.getSrcField() != null) {
			attributes.put("href", oConvertUtils.getString(reflectHelper
					.getMethodValue(comboTreeModel.getSrcField())));
		...
		if (in == null) {
			...
			if (in.size() > 0) {
				for (Object inobj : in) {
					...
					String inId = oConvertUtils.getString(reflectHelper2
							.getMethodValue(comboTreeModel.getIdField()));
					// update-begin--Author:JueYue Date:20140514
					// for：==不起作用--------------------
					if (inId.equals(id)) {
						tree.setChecked(true);
					}
					// update-end--Author:JueYue Date:20140514
					// for：==不起作用--------------------
				}
			}
		}
		...
		List curChildList = (List) reflectHelper.getMethodValue(comboTreeModel
				.getChildField());
		if (curChildList != null && curChildList.size() > 0) {
			...
			if (recursive) { // 递归查询子节点
				...
				for (Object childObj : nextChildList) {
					ComboTree t = comboTree(childObj, comboTreeModel, in,
							recursive);
					children.add(t);
				}
				tree.setChildren(children);
	...
	public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
		...
		for (Object obj : all) {
			...
			String id = oConvertUtils.getString(reflectHelper
					.getMethodValue(treeGridModel.getIdField()));
			String src = oConvertUtils.getString(reflectHelper
					.getMethodValue(treeGridModel.getSrc()));
			String text = oConvertUtils.getString(reflectHelper
					.getMethodValue(treeGridModel.getTextField()));
			if (StringUtils.isNotEmpty(treeGridModel.getOrder())) {
				String order = oConvertUtils.getString(reflectHelper
						.getMethodValue(treeGridModel.getOrder()));
				tg.setOrder(order);
			}
			tg.setId(id);
			if (treeGridModel.getIcon() != null) {
				String iconpath = TagUtil.fieldNametoValues(
						treeGridModel.getIcon(), obj).toString();
				if (iconpath != null) {
					tg.setCode(iconpath);
				} else {
					tg.setCode("");
			...
			if (treeGridModel.getParentText() != null) {
				Object ptext = TagUtil.fieldNametoValues(
						treeGridModel.getTextField(), obj);
				if (ptext != null) {
			...
			List childList = (List) reflectHelper.getMethodValue(treeGridModel
					.getChildList());
			...
			if (treeGridModel.getRoleid() != null) {
				String[] opStrings = {};
				List<TSRoleFunction> roleFunctions = findByProperty(
						TSRoleFunction.class, "TSFunction.id", id);

				if (roleFunctions.size() > 0) {
					for (TSRoleFunction tRoleFunction : roleFunctions) {
						TSRoleFunction roleFunction = tRoleFunction;
						if (roleFunction.getTSRole().getId().toString()
								.equals(treeGridModel.getRoleid())) {
				...
				List<TSOperation> operateions = findByProperty(
						TSOperation.class, "TSFunction.id", id);
				StringBuffer attributes = new StringBuffer();
				if (operateions.size() > 0) {
					for (TSOperation tOperation : operateions) {
						if (opStrings.length < 1) {
							attributes
									.append("<input type=checkbox name=operatons value="
											+ tOperation.getId()
											+ "_"
											+ id
											+ ">"
			...
			if (treeGridModel.getFieldMap() != null) {
				...
				for (Map.Entry<String, Object> entry : treeGridModel
						.getFieldMap().entrySet()) {
					Object fieldValue = reflectHelper.getMethodValue(entry
							.getValue().toString());
			...
			if (treeGridModel.getFunctionType() != null) {
				String functionType = oConvertUtils.getString(reflectHelper
						.getMethodValue(treeGridModel.getFunctionType()));
