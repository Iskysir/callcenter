...
//                update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
	...
	public StringBuffer end() {
		...
		sb.append("window.top.closeLoadingDialog();");
		sb.append("},");
		
		sb.append("onBeforeLoad:function(param){ window.top.showLoadingDialog();}, ");
		sb.append("loadMsg:'', ");
		...
		sb.append("<div id=\"" + name + "tb\" style=\"padding:3px;   width: 100%; height: auto\">");
		if(hasQueryColum(columnList)){
			sb.append("<div name=\"searchColums\" style=\"width: 100%; display: inline-block;\">");
			//如果表单是组合查询
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						//sb.append("<span style=\"float:left; width: 300px; display:-moz-inline-box;display:inline-block;\">");
						sb.append("<span style=\"MARGIN-TOP: 10px; DISPLAY: inline-block;\">");
						...
						sb.append("<span style=\"OVERFLOW: hidden; FONT-SIZE: 14px; VERTICAL-ALIGN: middle; WHITE-SPACE: nowrap; TEXT-OVERFLOW: ellipsis; COLOR: #666666; TEXT-ALIGN: right; PADDING-LEFT: 30px; DISPLAY: inline-block; -o-text-overflow: ellipsis \" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
						...
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\" class=\"search-input\" type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+" />");
							}
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			sb.append("<div class=\"findBox-new\">");
			sb.append("<span style=\"float:right;\">");
			//sb.append("<span style=\"float:right; margin-top:-16px !important;\">");
			//sb.append("<a href=\"#\" title=\"查询\" class=\"findbtn-new\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.query")));
			//sb.append("<a href=\"#\" title=\"重置\" class=\"resetbtn-new\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.reset")) );
			sb.append("<a href=\"#\" title=\"查询\" class=\"findbtn-new\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", ""));
			sb.append("<a href=\"#\" title=\"重置\" class=\"resetbtn-new\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", "") );
		...
		if(toolBarList.size()==0 && !hasQueryColum(columnList)){
			...
			sb.append("<div style=\"display: inline;float: left;\" class=\"datagrid-toolbar\">");			
		}
		sb.append("<span style=\"float:left;\" >");
		if(toolBarList.size()>0)
		{
			for (DataGridUrl toolBar : toolBarList) {
				sb.append("<a href=\"#\" class=\"easyui-linkbutton "+toolBar.getIcon()+"\" plain=\"true\" title=\""+toolBar.getTitle()+"\"  icon=\""+toolBar.getIcon()+"\" ");
				if(StringUtil.isNotEmpty(toolBar.getOnclick()))
				{
					...
					{
						if("新增".equals(toolBar.getTitle())||"编辑".equals(toolBar.getTitle())||"查看".equals(toolBar.getTitle())||"录入".equals(toolBar.getTitle()))
						{
							//sb.append("\'"+toolBar.getTitle()+" "+title+"\',");
							sb.append("\'"+toolBar.getTitle()+""+title+"\',");
						}
						else
						{
...
//            update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
			...
			if(column.getFormatter()!=null)
			{
				...
				//扩展自定义函数操作格式化
				sb.append(",formatter:function(value,rec,index){");
				sb.append("var forval  = '"+column.getFormatter()+"';");
				sb.append("try {if(typeof eval(forval) == 'function'){");
				sb.append("return eval(forval+\"(value,rec,index)\")}");
			
				sb.append("else{ return new Date().format('"+column.getFormatter()+"',value);}");
				sb.append("}catch(e){return new Date().format('"+column.getFormatter()+"',value);}}");
			}
			// 加入操作
			if (column.getField().equals("opt")) {
				sb.append(",formatter:function(value,rec,index){");
