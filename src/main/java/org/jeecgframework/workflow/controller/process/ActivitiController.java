...
import org.jeecgframework.workflow.model.diagram.CustomProcessDiagramGenerator;
...
public class ActivitiController {
	...
	public AjaxJson startOnlineProcess(String configId,String id,HttpServletRequest request) {
		...
		dataForm.put(WorkFlowGlobals.BPM_FORM_CONTENT_URL, "chrhcFormBuildController.do?ftlForm&tableName="+configId+"&mode=read&load=detail&id="+id);
...
//		}
	...
	public AjaxJson processComplete(HttpServletRequest request, Variable var) {
		...
		boolean bflag = taskJeecgService.checkUserTaskIsHuiQian(taskId);
		...
		try {
			...
			tpbpmLog.setOp_type(model);
			tpbpmLog.setOp_name(user.getRealName());
			tpbpmLog.setOp_time(DataUtils.gettimestamp());
			tpbpmLog.setMemo(request.getParameter("reason"));
			
			//add by fangxu-wang 20151019 获得业务表和业务id
			HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery().processInstanceId(processInstanceId).orderByProcessInstanceId().desc().singleResult();
			tpbpmLog.setBizid(historicProcessInstance.getBusinessKey());			
			
			TSBaseBus tsBaseBus =  systemService.findUniqueByProperty(TSBaseBus.class, "id",historicProcessInstance.getBusinessKey());
			tpbpmLog.setSrcTable(tsBaseBus.getTSBusConfig().getOnlineId());
	...
	public void taskHistoryList(@RequestParam("processInstanceId") String processInstanceId,
        ...
        for(Map<String,Object> m:startAndEndlist){
        	if("startEvent".equals(m.get("ACT_TYPE_"))){
        		rows.append("{'name':'"+m.get("ACT_ID_")+"','processInstanceId':'"+m.get("EXECUTION_ID_") +"','startTime':'"+m.get("START_TIME_")+"','endTime':'"+m.get("END_TIME_")+"','assignee':'"+StringUtils.trimToEmpty("")+"','deleteReason':'"+StringUtils.trimToEmpty("完成")+"'},");
        	}
        }
	...
	@RequestMapping(params = "taskHistoryListDatagrid")
	public void taskHistoryListDatagrid(@RequestParam("processInstanceId") String processInstanceId,
			HttpServletRequest request, HttpServletResponse response,DataGrid dataGrid) {
		int p = dataGrid.getPage();
		int r = dataGrid.getRows();
		int beginIndex = 0;
		int endIndex = 0;
		if(p == 1){
			beginIndex = 0;
		...
		List<Map<String,Object>> startAndEndlist = activitiDao.getActHiActinst(processInstanceId);
		int total = historyService
                .createHistoricTaskInstanceQuery()
                .processInstanceId(processInstanceId).list().size()+startAndEndlist.size();
        ...
        if(p == 1){
            ...
            for(Map<String,Object> m:startAndEndlist){
            	if("startEvent".equals(m.get("ACT_TYPE_"))){
            		rowsSb.append("{'name':'"+m.get("ACT_ID_")+"','processInstanceId':'"+m.get("EXECUTION_ID_") +"','startTime':'"+m.get("START_TIME_")+"','endTime':'"+m.get("END_TIME_")+"','assignee':'"+StringUtils.trimToEmpty("")+"','deleteReason':'"+StringUtils.trimToEmpty("完成")+"'},");
            	}
            }
        }
        List<HistoricTaskInstance> historicTasks = historyService
                .createHistoricTaskInstanceQuery()
                .processInstanceId(processInstanceId).orderByHistoricTaskInstanceStartTime().asc().listPage(beginIndex, endIndex);
        
        for(HistoricTaskInstance hi : historicTasks){
        	logger.info("****TaskId="+hi.getId()+"******TaskName="+hi.getName()+"*******Assignee="+hi.getAssignee());
        	
        	StringBuffer sb =new StringBuffer(""); 
        	sb.append("from TPBpmLog where bpm_id = ? and taskid=?");
        	List<TPBpmLog> logList = systemService.findHql(sb.toString(), hi.getProcessInstanceId(),hi.getId());
        	String memo = "";
        	if(!ListUtils.isNullOrEmpty(logList)){
        		memo = logList.get(0).getMemo();
        	}
        	String starttime = hi.getStartTime()==null?"":DateFormatUtils.format(hi.getStartTime(), "yyyy-MM-dd HH:mm:ss");
        	String endtime = hi.getEndTime()==null?"":DateFormatUtils.format(hi.getEndTime(), "yyyy-MM-dd HH:mm:ss");
        	//rowsSb.append("{'name':'"+hi.getName()+"','processInstanceId':'"+hi.getProcessInstanceId() +"','startTime':'"+starttime+"','endTime':'"+endtime+"','assignee':'"+StringUtils.trimToEmpty(hi.getAssignee())+"','deleteReason':'"+StringUtils.trimToEmpty(hi.getDeleteReason())+"'},");
        	rowsSb.append("{'name':'"+hi.getName()+"','processInstanceId':'"+hi.getProcessInstanceId() +"','startTime':'"+starttime+"','endTime':'"+endtime+"','assignee':'"+StringUtils.trimToEmpty(hi.getAssignee())+"','deleteReason':'"+memo+"'},");
        	//System.out.println(hi.getName()+"@"+hi.getAssignee()+"@"+hi.getStartTime()+"@"+hi.getEndTime());
        }  
        DecimalFormat df = new DecimalFormat("#.00");
        double a = Double.parseDouble(df.format(total))/ Double.parseDouble(df.format(r));
		...
		String rowStr = StringUtils.substringBeforeLast(rowsSb.toString(), ",");
