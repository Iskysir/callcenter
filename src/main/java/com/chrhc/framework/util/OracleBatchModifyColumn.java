...
public class OracleBatchModifyColumn {
	 ...
	 */
	public static void  modifyColumnToLowerForOracle(String tableSpace)
	{
		
		//SELECT * FROM ALL_TABLES WHERE OWNER='hotline'  获取所有表
		 BeanFactory factory = (BeanFactory) context;
		//ALTER TABLE T_S_FUNCTION RENAME COLUMN "FUNCTIONTYPE" TO "functiontype";
		 org.springframework.jdbc.core.JdbcTemplate templateObj=(org.springframework.jdbc.core.JdbcTemplate)factory.getBean("jdbcTemplate");
		    List<Map<String,Object>> tableList=templateObj.queryForList("select table_name from user_tables where tablespace_name='"+tableSpace+"'");
		    System.out.println(tableList);
			for(int i=0;i<tableList.size();i++)
			{
				Map<String,Object> table=tableList.get(i);
				String tableName=table.get("TABLE_NAME").toString();
				List<Map<String,Object>> fieldList=templateObj.queryForList("select column_name from user_tab_columns where table_name='"+tableName+"'");
				for(int j=0;j<fieldList.size();j++)
				{
					String column=fieldList.get(j).get("COLUMN_NAME").toString();
					String sql="ALTER TABLE "+tableName+" RENAME COLUMN \"{FUNCTIONTYPE}\" TO \"{functiontype}\"";
					sql=sql.replace("{FUNCTIONTYPE}",column);
					sql=sql.replace("{functiontype}",column.toLowerCase());
					try {
	...
	public static void  modifyColumnToUpperForOracle(String tableSpace,String table_prefix)
	{
		...
		//ALTER TABLE T_S_FUNCTION RENAME COLUMN "FUNCTIONTYPE" TO "functiontype";
		org.springframework.jdbc.core.JdbcTemplate templateObj=(org.springframework.jdbc.core.JdbcTemplate)factory.getBean("jdbcTemplate");
		List<Map<String,Object>> tableList=templateObj.queryForList("select table_name from user_tables where tablespace_name='"+tableSpace+"' and table_name like '"+table_prefix+"%'");
		System.out.println(tableList);
		for(int i=0;i<tableList.size();i++)
		{
			Map<String,Object> table=tableList.get(i);
			...
			for(int j=0;j<fieldList.size();j++)
			{
				...
				String sql="ALTER TABLE "+tableName+" RENAME COLUMN \"{FUNCTIONTYPE}\" TO \"{functiontype}\"";
				sql=sql.replace("{FUNCTIONTYPE}",column);
				sql=sql.replace("{functiontype}",column.toUpperCase());
				try {
					templateObj.execute(sql);
				}
				catch (Exception e)
	...
	{
		modifyColumnToUpperForOracle("HOTLINE2","");
		//modifyColumnToUpperForOracle("HOTLINE2","T_S_DATA_SOURCE");
