...
$(document).ready(function() {
	function e() {
		/*function h() { - 1 != g.indexOf("*#emo_") && (g = g.replace("*#", "<img src='" + basePath +"/resources/themes/hotline/chat/img/").replace("#*", ".gif'/>"), h());
		}
		var e = new Date,
		f = "";
		f += e.getFullYear() + "-",
		...
		var g = $("#editor").val();
		h();
		var i = "<div class='message'><div class='wrap-text'>" + "<h5 class='ask-name'>市民：路静</h5></div>" + "<div class='wrap-ri'><span>" + '('+f +')'+ "</span></div>" + "<div class='wrap-cont'>" + g + "</div>" + "</div>" + "<div class='message'><div class='wrap-text'>" + "<h5 class='question-name'>" + d + "</h5></div>" + "<div class='wrap-ri'><span>" + '('+f +')'+ "</span></div>"+"<div class='wrap-cont'>" + g + "\u7684\u56de\u590d\u5185\u5bb9</div>";
		null != g && "" != g ? ($(".mes" + a).append(i), $(".chat01_content").scrollTop($(".mes" + a).height()), $("#editor").val(""), message()) : alert("\u8bf7\u8f93\u5165\u804a\u5929\u5185\u5bb9!")*/
	}
	var a = 3,
	//b = "img/head/2024.jpg",
	//c = "img/head/2015.jpg",
	d = $(".chat-title h1 b").html();
	//窗口右上角关闭按钮
	...
	$(".min_btn").click(function(){
		$("#iframebox",window.parent.document).css("display","none");
		$("#chat-min-window",window.parent.document).css("display","block");
	});
	//内容显示区域提示信息点击关闭
	$(".talkTo li a").click(function(){
		$(".chat01_title").css("display","none");
	    $(".chat01_content").css("height","250px");
		})
	...
	$(".wl_faces_main img").click(function() {
		var a = $(this).attr("src");
		$("#editor").val($("#editor").val() + "*#" + a.substr(a.indexOf("img/") + 4, 6) + "#*"),
		$("#editor").focusEnd(),
		$(".wl_faces_box").hide()
	}),
