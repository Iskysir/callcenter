		...
		check : function(curform, brothers, subpost, bool) {
			...
			if(validType!=null){
		     ...
		     //  var  ajaxResultValue=Validform.util.ajax_check(params[0],params[1],$(this).val(),$("input[name='"+params[2]+"']").val());
		       var  ajaxResultValue=Validform.util.ajax_check(params[0],params[1],$("input[name='"+params[1]+"']").val(),$("input[name='"+params[2]+"']").val());
			   ...
			   if (resultParams[1] == "false" && tipType == 1) {
                	...
                	$(this).parent().find("span.Validform_checktip").html(resultParams[0]).addClass("Validform_checktip Validform_wrong");
                    //$(this).next("span.Validform_checktip").html(resultParams[0]);
                    //$(this).next().addClass("Validform_checktip Validform_wrong");
                    _this.addClass("Validform_error");
                    return false;
                }
		    }	
				   ...
				   var validType=$(this).attr("validType");
					if(validType!=null&&$(this).val()!=""){
					  ...
					  if(resultParams[1]=="false"){
						$(this).parent().find("span.Validform_checktip").html(resultParams[0]).addClass("Validform_checktip Validform_wrong");
						//20150706 fixed bug by mengfx 1、$(this) is not input;2、execution order is wrong
						/*if($("input#sfzh").length==1){
							setTimeout(function(){$("input#sfzh").parent().find("span.Validform_checktip").html(resultParams[0]).addClass("Validform_checktip Validform_wrong");}, 100);
						}else {
							setTimeout(function(){$(this).parent().find("span.Validform_checktip").html(resultParams[0]).addClass("Validform_checktip Validform_wrong");}, 100);
						}*/
			...
			curform
					...
					.each(
							function() {
								...
								//20150707 mengfx 如果判断不通过，不执行此段
								if(!$(this).parent().find("span.Validform_checktip").hasClass("Validform_wrong")){
									Validform.util.showmsg.call(curform,
											inflag.info, settings.tiptype, {
												obj : $(this),
												type : inflag.type,
