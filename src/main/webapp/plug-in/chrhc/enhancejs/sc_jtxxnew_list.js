function delObj(url,name) {
       ...
       data.zTable="sc_rkjbxxnew";
       data.wjIdName="ssjt_id";
	   $.ajax({
	    url:checkurl,
	    data:data,
		type:"Post",
...
//批量删除
	function sc_jtxxnewdelBatch(){
	
		//获取选中的ID串
		var ids = getsc_jtxxnewListSelections('id');
		if(ids.length<=0){
			tipAlert('请选择至少一条信息');
			return;
		}
	   ...
	   var checkurl="kinshipController.do?checkdeljt";
       var data = {};
       data.id=id;
       data.mainTable = mainTable;
       data.zTable="sc_rkjbxxnew";
	    ...
	    success:function(data){
			...
			if(data.success){
			 ...
			 $.ajax({
			    ...
			    data:{'ids':ids,'configId':'sc_jtxxnew'},
				type:"Post",
			    dataType:"json",
			    success:function(data){
					tip(data.msg);
					reloadsc_jtxxnewList();
			    },
				error:function(data){
					$.messager.alert('错误',data.msg);
				}
