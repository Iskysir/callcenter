	...
	if($('#formobj').Validform().check()){
		...
		$.post(
			    ...
			    function(data) {
					...
					reporturl= reporturl+"statement/亲属关系证明.cpt&id="+id+"&zip="+zip+"&d="+myt;
					parent.parent.addTab("亲属关系证明", encodeURI(reporturl));
					//parent.parent.addTab("打印亲属关系证明", "scCerTemplateController.do?tempPrint&id=" + id + "&tableName=sc_relations");
		});	
