	...
	if($('#formobj').Validform().check()){
		...
		$.post(
			    ...
			    function(data) {
					...
					reporturl= reporturl+"statement/无工作证明.cpt&id="+id+"&zip="+zip+"&d="+myt;
					parent.parent.addTab("打印无工作证明", encodeURI(reporturl));
					//parent.parent.addTab("打印无工作证明", "scCerTemplateController.do?tempPrint&id=" + id + "&tableName=sc_stay");
