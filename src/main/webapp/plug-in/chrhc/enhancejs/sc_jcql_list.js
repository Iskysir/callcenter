 function delObj(url,name) {
       ...
       data.zTable="sc_jcqlhdjl";
       data.wjIdName="team_name1";
	   $.ajax({
	    url:checkurl,
	    data:data,
		type:"Post",
...
//批量删除
	function sc_jcqldelBatch(){
	
		//获取选中的ID串
		var ids = getsc_jcqlListSelections('id');
		if(ids.length<=0){
			tipAlert('请选择至少一条信息');
			return;
		}
		delObjNew('确定要删除吗?', function(r) {
			$.ajax({
				url:"scJcqlController.do?doBatchDel&ids="+ids,
				type:"Post",
				dataType: 'text',
				success:function(data){
					...
					if (d.success) {
						...
						$.dialog.alert(d.msg,reloadsc_jcqlList);
					}
				},
				error:function(data){
