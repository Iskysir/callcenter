...
function openProcessDef(id) {
	$.ajax({
		url : "processController.do?getProcessXml",
		dataType : 'xml',
		data : {
			processId : id
		},
...
function createCanvas(processid,disabled) {
	...
	try {
		...
		if (typeof processDefinitionId != "undefined" && processDefinitionId != "0") 
...
function openProperties(id, turnpage, type) {
	...
	$('#properties-panel').panel('refresh', 'processController.do?processProperties&turn=' + turnpage + '&id=' + id + '&processId=' + processId + '&processDefinitionId=' + processDefinitionId);
}
//上一步
function redo() {
...
function saveProcessDef() {
	...
	$.ajax({
		url : "processController.do?saveProcess",
		type : 'POST',
		data : {
