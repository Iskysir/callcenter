		...
		"changeTips":function(value){
			...
			function valChange(){
				...
				if($this.val()==""){
					...
					if(se == 0){
						$('.on_changes').append('<li email="">没有匹配菜单..</li>');
					}
					else{
						$('.on_changes').append('<li email="">请选择菜单..</li>');
						
						for(var i in stu1) { 
							$('.on_changes').append('<li email="'+stu1[i].menu+'" img="'+stu1[i].img_small+'"></li>');
						}
					}
				
					$(value.divTip).show().children().each(function(index) {
							var img_sma = $(this).attr("img");
							var valAttr = '<img src="images/icons/small/'+img_sma+'"/>'+$(this).attr("email");
							if(index==1){$(this).html(valAttr).addClass("active").siblings().removeClass();}
								//索引值大于1的LI元素进处处理
								if(index>1){
									$(this).html(valAttr);
								}
