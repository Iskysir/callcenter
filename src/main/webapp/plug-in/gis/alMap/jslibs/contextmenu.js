//右键菜单控件
//params = {rightContextMenu:{
//								menuStyle:{backColor:"#FFFFFF",overColor: "#D6E9F8"},
//								isShowFindPath:false,isShowZoomIn:true,isShowZoomOut:false,isShowCenter:true,
//								selfDefineMenu:[{title:"",callBack:""}]}}
function createRightContextMenu(rightContextMenu,baseMap){
		if(rightContextMenu){
			//右键菜单样式
			 var style;
			 //右键菜单要素集合
			...
			//MenuStyle
			if(rightContextMenu.menuStyle){
				var menuStyle = rightContextMenu.menuStyle;
				var backColor = "#FFFFFF",
					overColor = "#D6E9F8";
				if(menuStyle){
					 backColor = menuStyle.backColor;
					 overColor = menuStyle.overColor;
				}
				  style = new SuperMap.MenuStyle({
						backColor:backColor,
						overColor: overColor,
						speed: 100});
			}
		   //isShowFindPath
		   if(rightContextMenu.isShowFindPath){
					_items.push(new SuperMap.MenuItem({
						title:"以此处为出发点的线路",
			   ...
			   //isShowZoomIn
		   if(rightContextMenu.isShowZoomIn){
					_items.push(new SuperMap.MenuItem({
						title: "放大",
						callBack:function(e,pos){
							baseMap.zoomIn();
			   ...
			   //isShowZoomOut
		   if(rightContextMenu.isShowZoomOut){
					_items.push(new SuperMap.MenuItem({
						title: "缩小",
						callBack:function(e,pos){
							baseMap.zoomOut();
			   ...
			   //isShowCenter
		   if(rightContextMenu.isShowCenter){
					_items.push(new SuperMap.MenuItem({
						title:"此处居中放置地图",
						callBack:function(e,pos){
							var px = new SuperMap.Pixel(pos.x,pos.y);
							var point = baseMap.getLonLatFromPixel(px);
							baseMap.panTo(point);
						}
						}));
		   }
		   _items.push(null);
			if(rightContextMenu.selfDefineMenu){
				var selfDefineMenu = rightContextMenu.selfDefineMenu;
				for(var i=0;i<selfDefineMenu.length;i++){
					var title = selfDefineMenu[i].title;
					var scallBack = selfDefineMenu[i].callBack;
					//alert(title+" "+callBacks[i]);
					_items.push(new SuperMap.MenuItem({
						title:title,
						callBack:scallBack
						}));
		...
		//alert(SuperMap.Util.toJSON(_items));
...
(function(SuperMap) {
	//panguso.maps.event.addListener(baseMap, 'contextmenu', function() {
	//	alert('baseMap rightclick');
	//});
	var timeCounter = 0;
	var j$ = jQuery;
	/*****************************Begin：jQuery右键菜单插件***************************************************/
...
//							evt.stopPropagation();
							$(this).mouseup( function(e) {
								...
								function showMenu(){
									...
									var date = new Date();
									if(date.getTime()-timeCounter<80){
										date = null;
										return;
									}
//									e.stopPropagation();
		...
		_useContextMenu:function(){
			...
			j$(this.mapContainer).contextMenu(args,
				function(action, el, pos) {
					//alert(" main.items.length="= main.items.length);
