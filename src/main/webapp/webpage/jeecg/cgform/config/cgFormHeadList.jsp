...
<t:datagrid sortName="createDate" sortOrder="desc" name="tablePropertyList" title="smart.form.config" actionUrl="cgFormHeadController.do?datagrid" idField="id" fit="true" queryMode="group" checkbox="true">
	...
	<t:dgCol title="编号" field="code" query="true" autocomplete="true" width="40"/>
	<t:dgCol title="table.name" field="tableName" query="true" autocomplete="true" width="40"/>
	<t:dgCol title="table.description" field="content" query="true"  width="40"></t:dgCol>
	...
	<t:dgCol title="common.createby" field="createName" hidden="false" query="true"></t:dgCol> --%>
	<t:dgCol title="common.createtime" field="createDate" formatter="yyyy/MM/dd" width="30"></t:dgCol>
	...
	<t:dgFunOpt exp="isDbSynch#eq#Y&&jformType#ne#3" funname="addlisttab(code,content)" title="function.test"></t:dgFunOpt>
	<t:dgFunOpt exp="isDbSynch#eq#Y&&jformType#ne#3" funname="popMenuLink(code,content)" title="config.place"></t:dgFunOpt>
	<t:dgToolBar title="create.form" icon="icon-add" width="900" height="800" url="cgFormHeadController.do?addorupdate" funname="addForm"></t:dgToolBar>
	<t:dgToolBar title="edit.form" icon="icon-edit" width="900" height="800" url="cgFormHeadController.do?addorupdate" funname="updateForm"></t:dgToolBar>
	<t:dgToolBar title="custom.button" icon="icon-zdy" url="cgformButtonController.do?cgformButton" funname="cgFormButton"></t:dgToolBar>
	<t:dgToolBar title="js.enhance" icon=" icon-js" url="cgformEnhanceJsController.do?addorupdate" funname="enhanceJs"></t:dgToolBar>
	<t:dgToolBar title="sql.enhance" icon="icon-sql" url="cgformButtonSqlController.do?addorupdate" funname="cgFormButtonSql"></t:dgToolBar>
	<t:dgToolBar title="form.export" icon="icon-putout" url="cgformSqlController.do?doMigrateOut" funname="doMigrateOut"></t:dgToolBar>
	<t:dgToolBar title="form.import" icon="icon-import" url="cgformSqlController.do?inSqlFile" funname="toCgformMigrate"></t:dgToolBar>
	<t:dgToolBar title="code.generate" icon="icon-code" url="generateController.do?gogenerate" funname="generate"></t:dgToolBar>
	<t:dgToolBar title="form.generate" icon="icon-db" url="cgformTransController.do?trans" funname="addToData"></t:dgToolBar>
</t:datagrid></div>
</div>
...
<script type="text/javascript">
	...
	function addlisttab(tableName,content){
		addOneTab( '<t:mutiLang langKey="form.datalist"/>' + "["+content+"]", "chrhcAutoListController.do?list&id="+tableName);
	}
	...
	*/
	function popMenuLink(tableName,content){
        $.dialog({
			content: "url:cgFormHeadController.do?popmenulink&url=chrhcAutoListController.do?list&title="+tableName,
			drag :false,
			lock : true,
