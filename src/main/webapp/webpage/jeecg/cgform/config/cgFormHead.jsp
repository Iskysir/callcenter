...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" beforeSubmit="deleteUnUsedFiled();" layout="table" tiptype="1" action="cgFormHeadController.do?save">
	...
	<table cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value" colspan="3">
			  <input class="inputxt" id="code" name="code" value="${cgFormHeadPage.code}"
				prefixName="" <c:if test="${not empty cgFormHeadPage.code}">readonly="readonly"</c:if> datatype="*" validType="cgform_head,code,id" nullmsg=请输入编号>  
		...
		<tr>
			...
			<td class="value" colspan="3">
			  <input class="inputxt" id="tableName" name="tableName" value="${cgFormHeadPage.tableName}"
				prefixName="" <c:if test="${not empty cgFormHeadPage.tableName}">readonly="readonly"</c:if> datatype="*"  nullmsg=<t:mutiLang langKey="please.input.table.name"/>> 
				<span class="Validform_checktip"></span></td>
		</tr>
		<tr>
		...
		<c:if test="${cgFormHeadPage.jformType eq '2'}">
			<tr id="fb_tb">
				<td align="right"><label class="Validform_label">关联表单：</label></td>
				<td class="value" colspan="3"><input class="inputxt" style="width: 440px" disabled="disabled" value="${cgFormHeadPage.subCode}"></td>
			</tr>
			<tr id="fb_tb">
				<td align="right"><label class="Validform_label">关联子表：</label></td>
				<td class="value" colspan="3"><input class="inputxt" style="width: 440px" disabled="disabled" value="${cgFormHeadPage.subTableStr}"></td>
			</tr>
		</c:if>
		<tr>
			<td align="right"><label class="Validform_label"> 卡片打开宽度: </label></td>
			<td class="value"><input name="cardWidth" value="${cgFormHeadPage.cardWidth}"/></td>
			<td align="right"><label class="Validform_label"> 卡片打开高度: </label></td>
			<td class="value"><input name="cardHeight" value="${cgFormHeadPage.cardHeight}"/></td>
		
		</tr>
		<tr>
			<td align="right"><label class="Validform_label"> 编号规则: </label></td>
			<td class="value" colspan="3"><input name="idkey" value="${cgFormHeadPage.idkey}"/>【规则key;字段名】(请按格式配置，规则ker对应编码规则配置的业务key ,字段对应表中字段名称，不填写默认为code)</td>
		</tr>	

		<tr>
			<td align="right"><label class="Validform_label">拦截器类名: </label></td>
			<td class="value" colspan="3"><input name="interClass" value="${cgFormHeadPage.interClass}"  size="80" /></td>
		</tr>		
		
		<tr>
			<td align="right"><label class="Validform_label"> 表单验证提示方式: </label></td>
			<td class="value" colspan="3">
			 <t:dictSelect field="tipType" type="list"
									typeGroupCode="tiptype" defaultVal="${cgFormHeadPage.tipType}" hasLabel="false"  title="表单验证提示方式"></t:dictSelect>     
			</td>
 		</tr>
  ...
  <div id="tabs" class="easyui-tabs" tabPosition="top" fit="false" style="margin: 0px; padding: 0px; overflow: hidden; width: auto;">
    <div title= '<t:mutiLang langKey="database.property"/>' width="auto" style="width: auto; margin: 0px; padding: 0px; overflow: hidden;">
      <div style="height: 25px;position:absolute;right:2px;" class="datagrid-toolbar"><a id="addColumnBtn" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="addColumnBtnClick();" href="#"><t:mutiLang langKey="common.add.to"/></a> <a
