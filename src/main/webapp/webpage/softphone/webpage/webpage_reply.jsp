	...
	<div class="col-md-12">
		<div class="row">
			<div class="grid simple">
				...
				<form name="inputFormReply" id="inputFormReply" method="post">
		...
		<div class="col-md-12 footer-btn padding-25 m-t-40">
            <div class="col-md-6">
				<c:choose>
					<c:when test="${empty channelRecord.obligateC}">
						<button type="button" class="btn btn-primary" onclick="addOrder('${channelRecord.id}');">新建工单</button>
						<%-- <button type="button" class="btn btn-primary" onclick="toRelate('${record.id}');">关联工单</button> --%>
					</c:when>
					<c:otherwise>
						<button type="button" class="btn btn-primary" onclick="viewOrder('${channelRecord.obligateC}');">查看工单</button>
					</c:otherwise>
				</c:choose>
				
	            <input type="button" id="submitBtn" value="提交" onclick="webpageReplySubmit()" class="btn btn-primary"/>
	            <input type="button" id="submitBtn" value="重填" onclick="refreshReplyInput()" class="btn btn-default"/>
	...
	<script type="text/javascript">
		$(document).ready(				
				function() {	
					var is = $("#isPublic").val();
					if(is == '0'){
						$("#inputFormReply label[for='searchCode']")
						.replaceWith(
		...
		$(function() {
			...
			var validator = $("#inputFormReply").validate(
					{
		...
		//提交保存数据
		 function webpageReplySubmit() {	
			 	var id= $("#id").val();	
			 	var reply = $("#replyContent").val();	
					$.ajax({
						type : "post",
						url : "${ctx}/channelrecord/wp/saveWebPageReply/"+id,
		...
		//重填按钮
		function refreshReplyInput(){
			var id=$("#id").val();
			uiBase.addOneTab('处理',"${ctx}/channelrecord/wp/toReplyWebPage/"+id);
		}
		//工单新增
		function addOrder(id){
			uiBase.addOneTab('工单新增',"${ctx}/channelrecord/wp/addOrderWebPage/ChannelTypeWebPage/" + id );	
		}
		//查看工单
