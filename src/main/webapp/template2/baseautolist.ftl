...
function createDataGrid${config_id}(){
	var initUrl = 'chrhcAutoListController.do?datagrid&configId=${config_id}&field=${fileds}${initquery}&biztype=${bizType!}';
	...
	columns:[
		[	
			...
			{field:'opt',title:'操作',width:200,formatter:function(value,rec,index){
						...
						href+="[<a href='#' onclick=delObj('chrhcAutoListController.do?del&configId=${config_id}&id="+rec.id+"','${config_id}List')>";
						href+="删除</a>]";
						</#if>
	...
	function ${config_id}Listsearch(){
		...
		$('#${config_id}Listtb').find('*').each(
			...
			$('#${config_id}List').<#if config_istree=="Y">treegrid<#else>datagrid</#if>({url:'chrhcAutoListController.do?datagrid&configId=${config_id}&field=${fileds}&biztype=${bizType!}',pageNumber:1});
	}
	...
	function ${config_id}add(){
		//parent.addTab
		add('新增<#if bizTypeText?? && bizTypeText != ''>${bizTypeText}</#if>${config_name}','chrhcFormBuildController.do?ftlForm&tableName=${config_id}&bizType=${bizType!}${initquery}','${config_id}List',${config_id}Fw,${config_id}Fh);
	}
	//修改
	function ${config_id}update(){
		update('编辑<#if bizTypeText?? && bizTypeText != ''>${bizTypeText}</#if>${config_name}','chrhcFormBuildController.do?ftlForm&tableName=${config_id}&bizType=${bizType!}${initquery}','${config_id}List',${config_id}Fw,${config_id}Fh);
	}
	//查看
	function ${config_id}view(){
		detail('查看<#if bizTypeText?? && bizTypeText != ''>${bizTypeText}</#if>${config_name}','chrhcFormBuildController.do?ftlForm&tableName=${config_id}&mode=read&bizType=${bizType!}','${config_id}List',${config_id}Fw,${config_id}Fh);
	}
	...
	function ${config_id}ExportExcel(){
		...
		window.location.href = "excelTempletController.do?exportXls&tableName=${config_id}&bizType=${bizType!}"+encodeURI(params+fields)
	}
...
<#if config_querymode == "group">
	<#list config_queryList  as x> 
		...
		<span style="vertical-align:middle;display:-moz-inline-box;display:inline-block;padding-left:30px;text-align:right;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap;" title="${x['field_title']}">${x['field_title']}：</span>
		</#if>
	...
	<a id="detail" href="#" class="easyui-linkbutton" plain="true"  icon="icon-see" onclick="${config_id}view()">查看</a>
